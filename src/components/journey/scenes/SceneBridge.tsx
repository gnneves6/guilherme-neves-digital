...
const SceneBridge = ({ index = 2 }: { index?: number }) => {
  return (
    <Scene
      index={index}
      tone="hsl(var(--background))"
      overlay="radial-gradient(ellipse 100% 70% at 50% 50%, hsl(var(--background)) 0%, hsl(var(--background) / 0.92) 100%)"
      zoomFrom={1.04}
      zoomTo={1.0}
    >
      <div className="absolute inset-0 section-padding grid place-items-center">
        <ParallaxLayer strength={-8} className="max-content w-full">
          <TitleCard number="03" label="Bridge" tone="light" className="mb-10" />
          <div className="grid md:grid-cols-[1.1fr_1fr] gap-10 md:gap-16 items-start">
            <div>
              ...
              />
              <p className="mt-6 md:mt-8 font-sans text-base md:text-lg leading-relaxed max-w-xl" style={{ color: "hsl(var(--foreground) / 0.78)" }}>
                Before I studied performance, I lived the routine: training,
                competition, recovery, pressure and the small decisions that
                shape consistency.
              </p>
              <p className="mt-4 font-display italic text-base md:text-lg max-w-xl" style={{ color: "hsl(var(--foreground) / 0.6)" }}>
            ...
            <div className="grid grid-cols-3 gap-4 md:gap-6 md:pt-2">
              {pillars.map((p, i) => (
                ...
                >
                  <span className="font-display text-[10px] tracking-[0.4em]" style={{ color: "hsl(var(--olive))" }}>
                    {p.num}
                  </span>
                  <span className="mt-3 font-display text-sm md:text-base leading-snug text-foreground">{p.label}</span>
                  <motion.div
