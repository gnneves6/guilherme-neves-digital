...
const CredibilityStrip = () => {
  return (
    <section
      ...
      className="relative border-t border-b border-foreground/5 bg-background"
    >
      <div className="mx-auto max-w-6xl px-6 py-16 md:py-20">
        <Reveal direction="up" duration={0.7}>
          <p className="text-[0.7rem] tracking-[0.28em] uppercase text-foreground/40 mb-6">
            Field Experience
          </p>
        </Reveal>
        ...
        <Reveal direction="up" delay={0.1} duration={0.7}>
          <p className="mt-8 text-sm md:text-[0.95rem] text-foreground/50 leading-relaxed flex flex-wrap gap-x-3 gap-y-2">
        ...
        <Reveal direction="up" delay={0.15} duration={0.7}>
          <ul className="mt-10 flex flex-wrap gap-x-2 gap-y-2">
            {chips.map((chip) => (
              <li
                key={chip}
                className="text-[0.7rem] tracking-[0.14em] uppercase text-foreground/45 border border-foreground/10 rounded-full px-3 py-1.5"
              >
                {chip}
              </li>
            ))}
